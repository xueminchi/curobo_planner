...
def demo_pick_and_place_fixed():    
    ...
    motion_gen_config = MotionGenConfig.load_from_robot_config(
        robot_file,
        world_config,  # 障碍物传给motion_gen
        tensor_args,  # 数据类型
        interpolation_dt=0.02,  # 插值时间
        collision_checker_type=CollisionCheckerType.PRIMITIVE,  # 碰撞检测类型
    ...
    # 创建可视化器
    visualizer = PickAndPlaceVisualizerFixed(gui=True)  # Pybullet可视化器
